; homeu.g
; called to home the X axis
;
; generated by RepRapFirmware Configuration Tool v3.1.10+4-LPC on Tue Jan 19 2021 17:25:01 GMT+0800 (中国标准时间)
G91               ; relative positioning
G1 H2 Z3 F6000    ; lift Z relative to current position
G1 H1 U300 F1800 ; move quickly to X axis endstop and stop there (first pass)
G1 H2 U-5 F6000    ; go back a few mm
G1 H1 U300 F360  ; move slowly to X axis endstop once more (second pass)
G1 H2 Z3 F6000   ; lower Z again
G90               ; absolute positioning

